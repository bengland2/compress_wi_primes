...
impl BitString for DynBitString {
    fn null() -> Self {
        DynBitString { bitcnt: 0, b: Vec::new() }
    }

    fn len(&self) -> usize { self.bitcnt }

    fn get(&self, ndx: usize) -> bool {
        assert!(ndx < self.bitcnt);
    ...
    fn clip(&mut self, newsz: usize) {
        ...
        if newsz < self.bitcnt {
            // don't shrink vector but zero out bits from newsz to end
            for k in newsz..self.bitcnt {
                self.set(k, false);
            }
            self.bitcnt = newsz;
        } else if newsz > self.bitcnt {
            let old_bitcnt = self.bitcnt;
    ...
    fn append(&mut self, bit: bool) {
        if self.bitcnt % BITS_PER_BYTE == 0 {
            if self.b.len() * BITS_PER_BYTE == self.bitcnt {
                self.b.push(0);  // allocate another 8 bits
            } else {
                assert!(self.b.len() * BITS_PER_BYTE > self.bitcnt);
            }
        }
        self.bitcnt += 1;
        let byte_index = (self.bitcnt - 1) / BITS_PER_BYTE;
        let mut last_byte = self.b[byte_index];
        let bit_within_byte = (self.bitcnt - 1) % BITS_PER_BYTE;
...
impl PartialEq for DynBitString {
    fn eq(&self, other: &Self) -> bool {
        if self.bitcnt != other.bitcnt { false }
        else if self.bitcnt == 0 { true }
        else {
            for k in 0..self.bitcnt {
